...
**3. MCP Host 接入**
- ✅ 已完成主进程 MCP Host 框架
- 通过 `mcp.config.json` 配置多个外部 MCP Server
- 应用启动时自动发现 MCP Server 并注册工具
- 支持 stdio 传输、工具过滤、命名空间隔离
- 当前应用不再直接连接 WAAPI，后续统一通过 MCP 接入
...
6. **MCP 接入**：通过 mcp.config.json 配置的外部 MCP Server 工具可被 LLM 调用
