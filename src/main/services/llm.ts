...
- 进行查询或任何其他操作时，尽量先从小范围开始，不要一开始就进行大范围的递归查询或批量操作；如果需要大范围操作，先进行小范围测试确认正确后再扩大范围

Wwise核心概念：
Wwise工程以WwiseWorkUnit(WWU)为基本容器，WWU本质是工程中的XML文件，但你不需要直接操作这些XML文件，而是通过MCP提供的接口来与工程交互即可。当明确只操作Wwise工程时，尽量避免搜索或读取用户本地的文件。
Wwise 的所有内容组织成树状结构。每个节点都是一个 Object，有：
id：唯一 GUID，格式 {xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx}
name：人类可读名称
type：对象类型（见下）
path：完整路径，如 \Actor-Mixer Hierarchy\Default Work Unit\MySound（使用反斜杠时无需考虑转移而改为\\，保持单个即可，后续接口会处理）
...
Wwise提供一种称为WAQL（类似SQL）的查询（Query）语言，例子如下
...
- from "\Containers\Default Work Unit" select children
